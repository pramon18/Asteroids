...
def create_Asteroid(name, color):
	...
	asteroid_life = random.randint(10,500)
	asteroidrect.center = (pos_x,pos_y)

	'now we return'
	return asteroid, asteroidrect, asteroid_Angle, a_velocity_x, a_velocity_y, asteroid_life
...
while True:
	...
	'asteroids image files'
	a_img = ["../images/asteroid_Big_One.bmp", "../images/asteroid_Big_One.bmp", "../images/asteroid_Big_One.bmp"]
	...
	if state == "playing":
		...
		while True:
			...
			shot = pygame.mixer.Sound("../sounds/lasershot.ogg")

			if pressed_Keys[pygame.K_SPACE]:
				if (pygame.time.get_ticks() - tempo) >= 50:
					tempo = pygame.time.get_ticks()
					if len(b) < 50:
						b.append(list(create_Bullet("../images/bullet.bmp",(135,254,255), ship_Angle)))					
						shot.set_volume(0.1)
						shot.play()
				else:
					pass
			...
			if seconds % 2 == 0 and cont == 1 and len(a) <= 0:
				asteroid_image = random.choice([0,1,2])
				a.append(list(create_Asteroid(a_img[asteroid_image],(135,254,255))))
				print(a[0])
				cont = 0
			...
			'list to get all the bullet rects'
			b_rect = []
			...
			for i in b:
				...
				if new_b_x > 640 or new_b_x < 0 or new_b_y > 480 or new_b_y < 0:
					...
					b_rect.append(bullet_copy_rect)
			...
			for j in a:
				...
				new_a_x = j[1].center[0] + (j[3]*0.1)
				new_a_y = j[1].center[1] + (j[4]*0.1)
			...
			if asteroid_copy_rect.collidelist(b_rect) == -1:
				pass
			else:
				b.pop(asteroid_copy_rect.collidelist(b_rect))
			...
			if asteroid_copy_rect.colliderect(shiprect):
				a.pop(a.index(j))
				state = "game over"
				print(state)
				break
		...
		screen.blit(text_font.render("GAME OVER", True, (255,255,255)),(240,370))
		screen.blit(text_font.render("Your SCORE is: "+str(score), True, (255,255,255)),(240,390))
